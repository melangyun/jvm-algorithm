...
class BOJ2493 {
    ...
    fun solution() {
        ...
        val buildings = br.readLine().split(" ").map { it.toInt() }
        val answer = StringBuilder()

        (num - 1).downTo(1).forEach {
            for (i: Int in (it - 1).downTo(0)) {
                if (buildings[i] >= buildings[it]) {
                    answer.append("${i + 1} ")
                    break
                }
                if (i == 0) {
        ...
        return print(answer.append("0").reverse())
    }
    ...
    fun solution2(){
        ...
        stack.add(Pair(1, buildings[0]))

        for (i in 1 until num) {
            while (stack.isNotEmpty()) {
                // first: index, second: height
                val (first, second) = stack.last()
                if (second > buildings[i]) {
                    answer.append("$first ")
                    break
                }
                stack.removeLast()
            ...
            stack.add(Pair(i + 1, buildings[i]))
        }

        return print("0 " + answer)
    }
